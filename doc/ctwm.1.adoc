...
key.  `mod1` is the same as `meta`.  `mod2..5` can have various special meanings; try running `xmodmap
-pm` to see how your X server is mapping things (the `xkeycaps` program
may also be useful).  The `alter1..5` modifiers refer to ctwm alternate
...
f.altkeymap `number`::
  ...
  between 1 and 5 included. The next key or button event ctwm receives will
