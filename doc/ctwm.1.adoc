...
BackingStore::
  ...
  also `NoBackingStore`.  Added in %%NEXT%%; in previous versions this
  was the default.
...
DecorateTransients::
  ...
  `WM_TRANSIENT_FOR` property) should have titlebars.  As of %%NEXT%%
  this is the default.
...
ForceFocus [{ `win-list` }]::
  ...
  treatment.
+
First appeared in %%NEXT%%.  In prior versions, a variant of this
...
GrabServer::
  ...
  `NoGrabServer`.  Added in %%NEXT%%; in previous versions this was the
  default.
...
NoBackingStore::
  ...
  See also `BackingStore`.  As of %%NEXT%% this is the default.
...
NoDecorateTransients::
  ...
  also `DecorateTransients`.  Added in %%NEXT%%; in previous versions
...
NoGrabServer::
  ...
  when popping up menus and moving opaque windows.  As of %%NEXT%% this
  is the default.
...
NoRestartPreviousState::
  ...
  details.  Added in %%NEXT%%; in previous versions this was the default.
...
NoSortIconManager::
  ...
  order they show up.  See also `SortIconManager`.  Added in %%NEXT%%; in
...
+
As of %%NEXT%% this is enabled by default.
...
OpaqueMoveThreshold `threshold`::
  ...
  opaque mode.  The default is 200.  Since %%NEXT%%, values >= 200 are
...
OpaqueResize [{ `window-list` }]::
  ...
  `NoOpaqueResize` counterpart is also available.  As of %%NEXT%% this is
  enabled by default.
...
+
As of %%NEXT%%, ``on'' is the default.  Previously ``off'' was default.
...
RestartPreviousState::
  ...
  workspaces a window should be visible.  As of %%NEXT%% this is the
  default.
...
RplaySoundHost `string`::
  ...
  SOUNDS section.  Added in %%NEXT%%; prior versions used the `SoundHost`
  parameter instead.
...
SortIconManager::
  ...
  the end.  As of %%NEXT%% this is the default.

SoundHost `string`::
  Alias for `RplaySoundHost`, used in versions prior to %%NEXT%%.  Unless
...
f.priorityswitching::
  Toggle the window's switching ability.  X-ref `OnTopPriority` variable.
+
First appeared in %%NEXT%%.
...
As of %%NEXT%%, the sounds may be configured in the ctwmrc with the
