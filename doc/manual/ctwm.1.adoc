...
HOSTNAME::
  As of %%NEXT%%, this is a duplicate of `CLIENTHOST`.  In prior
...
DontNameDecorations::
  ...
  is known to be one of them).  First appeared in %%NEXT%%.
...
f.bottomzoom::
  ...
  resizes the window to fill only the bottom half of the current monitor.
...
f.fullscreenzoom::
  ...
  window to the full size of the current monitor or else restores the original
  size if the window was already zoomed. The frame is moved just outside the
  monitor.  This gives the appearance that window is shown without the frame:
  since ctwm can't dynamically make a window borderless, this is the next best
  thing.

f.fullzoom::
  This function resizes the selected window to the full size of the current
  monitor or else restores the original size if the window was already zoomed.
...
f.horizoom::
  ...
  selected window is resized to the full width of the current monitor.
...
f.leftzoom::
  ...
  the selected window is only resized to the left half of the current monitor.
...
f.rightzoom::
  ...
  the selected window is only resized to the right half of the current monitor.
...
f.topzoom::
  This variable is similar to the `f.bottomzoom` function except that
  the selected window is only resized to the top half of the current monitor.
...
f.xbottomzoom::
  This function is similar to the `f.xfullzoom` function, but
  resizes the window to fill only the bottom half of the screen (across all
  monitors).

f.xfullscreenzoom::
  This function resizes the client part (the inside the frame) of the selected
  window to the full size of the display (across all monitors) or else
  restores the original size if the window was already zoomed. The frame is
  moved just outside the screen. This gives the appearance that window is
  shown without the frame: since ctwm can't dynamically make a window
  borderless, this is the next best thing.

f.xfullzoom::
  This function resizes the selected window to the full size of the display
  (across all monitors) or else restores the original size if the window was
  already zoomed.

f.xhorizoom::
  This variable is similar to the `f.xzoom` function except that the selected
  window is resized to the full width of the display (across all monitors).

f.xhtzoom::
  This function is a synonym for `f.xtopzoom`.

f.xhzoom::
  This function is a synonym for `f.xhorizoom`.

f.xleftzoom::
  This variable is similar to the `f.xbottomzoom` function but causes the
  selected window is only resized to the left half of the display (across all
  monitors).

f.xrightzoom::
  This variable is similar to the `f.xbottomzoom` function except that the
  selected window is only resized to the right half of the display (across all
  monitors).

f.xtopzoom::
  This variable is similar to the `f.xbottomzoom` function except that the
  selected window is only resized to the top half of the display (accross all
  monitors).

f.xvlzoom::
  This function is a synonym for `f.xleftzoom`.

f.xvrzoom::
  This function is a synonym for `f.xrightzoom`.

f.xzoom::
  This function is similar to the `f.xfullzoom` function, except that
