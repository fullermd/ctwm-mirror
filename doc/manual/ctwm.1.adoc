...
[normal]
  ...
  ctwm will reject that, as was standard behavior prior to %%NEXT%%.
...
Windows get their named via X properties.  There are two types of names;
...
Prior to %%NEXT%%, ctwm only supported the standard ICCCM properties for
setting the window and icon names.  These are `WM_NAME` for the window
name, and `WM_ICON_NAME` for the icon name.  As of %%NEXT%%, ctwm also
...
As of %%NEXT%%, ctwm also supports an additional pair of properties;
...
a plain uninterpreted string (probably ASCII), and `8t` for
``internationalized'' (ICCCM-style locale conversion).  Usually you'd
just use UTF-8 though.  `xprop -remove CTWM_WM_ICON_NAME` and click would
let you undo it and go back to the normal naming.  See the `xprop(1)`
manual for more.
