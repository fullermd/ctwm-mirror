...
static struct _trmmbxinfo {
	...
	MbxIOSB
	trmIOSB;        /* termination mailbox I/O status block */
...
static unsigned char
wsaName[20];       /* VMS null terminate name for WorkStation Device */
static unsigned short TrmMbxBufferLen =
        ACC$K_TERMLEN;  /* length of the termination's mailbox */
...
void TrmProcedure()
{
	...
	if(accptr->msgtype != MSG$_DELPROC) {
		goto ReQueueIt;
	}
...
try_again:
	...
	TrmMbxInfo.trmMbxUnit = (unsigned short int)
	                        mbxunit;        /* set the unit number */
...
char *command;
{
	...
	if(!(istatus & 1)) {
		return (istatus);        /* return on error */
	}
	...
	if(iosb.bytes != strlen(STOP_CMD2)) {
		return (SS$_MBFULL);
	}
	...
	do {
		...
		istatus = sys$creprc(
		                  ...
		                  (PRC$M_DETACH | PRC$M_INTER |
		                   PRC$M_NOPASSWORD));   /* process status flags         */
